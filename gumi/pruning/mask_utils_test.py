...
class MaskUtilsTest(unittest.TestCase):
    ...
    def test_get_criterion(self):
        ...
        self.assertTrue(torch.allclose(mask_utils.get_criterion(W), W.norm(dim=(2, 3))))
    ...
    def test_get_sort_perm(self):
        ...
        self.assertTrue(
            np.allclose(mask_utils.get_sort_perm(C, dim=1), np.argsort(C.sum(axis=0)))
        )
    ...
    def test_permute_criterion(self):
        ...
        C_ = C.numpy()
        self.assertTrue(np.allclose(C_, mask_utils.permute_criterion(C, "SAME")[0]))
        self.assertFalse(np.allclose(C_, mask_utils.permute_criterion(C, "SORT")[0]))
    ...
    def test_run_mbm_with_perm(self):
        ...
        G = 4
        gnd_in1, gnd_out1, cost1 = mask_utils.run_mbm(W, G, perm=None, num_iters=1)
