...
parser.add_argument('--strategy',
                    type=str,
                    default='ILP',
                    help='How to solve the optimisation problem.')
...
parser.add_argument('--allow-depthwise',
                    action='store_true',
                    default=False,
                    help='Allow depthwise')
parser.add_argument('--absolute',
                    action='store_true',
                    default=False,
                    help='Use absolute cost')
parser.add_argument('--max-num-params',
                    type=float,
                    default=100,
                    help='Maximal number of parameters (M)')
parser.add_argument('--max-num-ops',
                    type=float,
                    default=1000,
                    help='Maximal number of operations (M)')
...
class GOpt(object):
  ...
  def ilp_solve(self, c_dict, max_num_params, num_num_ops):
    ...
    prob = LpSolver('Maximize cost under params and ops constraints',
                    LpMaximize)
    ...
    for name, (Gs, costs, norm) in c_dict.items():
      ...
      N = len(Gs)
      lp_bins[name] = LpVariable.dicts('{}_G=%s'.format(name), N, 0, 1,
                                       LpInteger)
  ...
  def run_opt(self,
    ...
    if strategy == 'ILP':
      ...
      state, cost = self.max_cost_solver(model, state, c_dict, max_num_params,
                                         max_num_ops)
    else:
