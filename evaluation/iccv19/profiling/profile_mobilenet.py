...
def generate_sparse_matrix(num_rows, num_cols, groups, block=True):
  ...
  if block:
    ...
    logging.info(
        '==> Generating random sparse matrix {} x {} with {:.2f}% sparsity ...'.
        format(num_rows, num_cols, 1.0 - nnz / (num_cols * num_rows)))
    ...
    for i in range(num_rows):
      if cur_nnz == 0:
        ...
        col_nnz = np.random.randint(cur_nnz // (num_rows - i),
                                    min(num_cols, cur_nnz))
  ...
  def profile(self, iters, use_cuda=False):
    ...
    for g in groups:
      for is_sparse in sparse:
        ...
        logging.info('==> Profiling G={} sparse={} for {} iters ...'.format(
            g, is_sparse, iters))
...
def main():
  ...
  df.to_csv(
      'data/profile/mobilenet_v1{}.csv'.format('_cuda' if args.
                                               use_cuda else ''),
      index=False)
