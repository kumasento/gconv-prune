...
class SparsifyDistillerRunner(ModelRunner):
  ...
  def get_sparsity(self, model):
    ...
    data = []
    columns = [
        'Name', 'Elem NNZ', 'Elem Total', 'Elem (%)', 'Kern NNZ', 'Kern Total',
        'Kern (%)'
    ]
    ...
    for name, mod in model.named_modules():
      if isinstance(mod, nn.Conv2d):
        ...
        logging.info(
            '==> Element-wise sparsity: nnz={} total={} sparsity={:.2f}%'.
            format(e_nnz, e_tot, 100 - e_nnz / e_tot * 100))
        ...
        data.append([
            name, e_nnz, e_tot, 100 - e_nnz / e_tot * 100, k_nnz, k_tot,
            100 - k_nnz / k_tot * 100
        ])
...
def main():
  ...
  print(df)
  print('Elem-wise sparsity: {:.2f}%'.format(
      (1 - df['Elem NNZ'].sum() / df['Elem Total'].sum()) * 100))
  print('Kern-wise sparsity: {:.2f}%'.format(
