...
class Profiler(object):
  ...
  def run_layer(self,
    ...
    logging.info(
        '==> Start timing of {in_channels:} x {out_channels:} x {kernel_size:} G={groups:} in mode={mode:} ...'
        .format(
    ...
    print(
        'Elapsed time: {:10.2f} sec (total) {:6.2f} ms (per run) {:6.2f} FPS.'.
        format(elapsed * 1e-3, elapsed / iters,
               iters * batch_size / elapsed * 1e3))
