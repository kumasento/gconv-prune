...
class Pruner:
    ...
    def regularize(self, *args, **kwargs):
        ...
        for epoch in range(epochs):
            print(
                "\nEpoch: [%d | %d] LR: %f" % (epoch + 1, epochs, self.state["reg_lr"])
            )
    ...
    def prune_module(self, name, mod, *args, g_cfg=None, fake_mask=False, **kwargs):
    ...
    def save_checkpoint(
        self, state, is_best, checkpoint="checkpoint", filename="checkpoint.pth.tar"
    ):
    ...
    def fine_tune(self, num_epochs):
        ...
        for epoch in range(self.args.epochs):
            ...
            self.save_checkpoint(checkpoint_state, is_best, checkpoint=self.checkpoint)

        # Finalising
        self.logger.close()
        logging.info("Best accuracy while fine-tuning: {:.2f}%".format(self.best_acc))
