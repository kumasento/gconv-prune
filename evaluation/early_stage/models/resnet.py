...
def conv1x1(in_planes, out_planes, stride=1, groups=1, indices=None, mask=False):
    """ 1x1 convolution """
    if not mask:
        return GroupConv2d(
            in_planes,
...
class Bottleneck(nn.Module):
  ...
  """
    ...
    """
        ...
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = conv3x3(
            planes, planes, stride=stride, groups=groups, indices=indices, mask=mask
        )
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv3 = conv1x1(
            planes, planes * self.expansion, groups=groups, indices=indices, mask=mask
        )
...
class ResNet(nn.Module):
  ...
  """
    ...
    def __init__(
        self, block, depth, groups=1, indices=None, mask=False, num_classes=10
    ):
        """ CTOR.
    Args:
