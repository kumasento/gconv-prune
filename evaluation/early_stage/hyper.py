...
parser.add_argument(
    ...
    type=str,
    metavar='PATH',
    help='Hyperparameter searching configuration.')
parser.add_argument(
    '--dir',
    type=str,
    metavar='PATH',
    help='Where to store the search results.')
...
class HyperParamSearcher(object):
  ...
  def __init__(self, args):
    ...
    logging.debug('The provided search space is: {}'.format(
  ...
  def run(self):
    ...
    logging.info('Workload in each queue: {}'.format(
        ...
        if tid not in task_to_proc[qid]:
          ...
          if proc.poll() is not None:  # execution finished
            ...
            if tid < len(args_list_queue[qid]):
              ...
              logging.info('Queue for GPU={} has bee processed.'.format(
                  self.gpu_ids[qid]))
    ...
    logging.info('Summary has been written to {}'.format(self.args.dir +
                                                         '/summary.json'))
