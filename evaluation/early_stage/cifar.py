...
parser.add_argument("--dataset-dir", default="data", help="Path to dataset", type=str)
parser.add_argument(
    "-j",
    "--workers",
    default=4,
...
parser.add_argument("--num-runs", default=-1, type=int, help="Number of random runs")
...
class Runner(object):
  ...
  """
    ...
    def get_logger(self, args):
        ...
        if args.resume:
            return Logger(
                os.path.join(args.checkpoint, "log.txt"), title=self.title, resume=True
            )
        else:
    ...
    """
        if self.args.evaluate:
            ...
            test_loss, test_acc = cifar_utils.test(
                self.test_loader, self.model, self.criterion, self.start_epoch, use_cuda
            )
        ...
        for epoch in range(self.start_epoch, self.args.epochs):
            ...
            print(
                "\nEpoch: [%d | %d] LR: %f" % (epoch + 1, self.epochs, self.state["lr"])
            )
            ...
            test_loss, test_acc = cifar_utils.test(
                self.test_loader, self.model, self.criterion, epoch, use_cuda
            )
...
def launch_random_run(args):
    ...
    new_args.checkpoint = args.checkpoint + ".{}".format(time.strftime("%Y%m%d%H%M%S"))
